...
**PRフロー**:
1. 実装タスク完了後、`gh pr create`コマンドでPRを作成（上記テンプレートを使用）
2. PRのURLとブランチ名をユーザーに報告
3. **自動的にレビュータスクを作成・実行**
   - タスクタイトル: `T{番号}-review: {元のタスク名}のレビュー`
   - base_branch: 実装タスクで使用したブランチ名（例: `vk/5470-t003-firestore-o`）
...
レビュータスクは実装タスクとは別の独立したタスクとして作成されます。

レビュー時には以下の方針でレビューを実施してください：
- 辛口にレビューをしてください。厳しめに、本来あるべき姿や、マサカリ大歓迎です
- そもそものベースの実装が改善の余地がある場合も指摘して欲しいです
...
**レビュー実施手順**:
1. 対象ブランチをチェックアウト
2. PRの差分を確認（`gh pr diff`または`git diff main...HEAD`）
3. 変更されたファイルを読み取り、実装内容を確認
4. 上記の観点に基づいて**1つの包括的なレビュー結果**を作成
   - **重要**: 複数の細かいコメントではなく、全ての指摘事項をまとめた1つのレビュー結果として提示すること
5. レビュー結果をユーザーに報告
6. **変更は少し待つ - ユーザーの指示を待つこと**
...
```
T001: infrastructure → T001-review（自動実行）
  ↓
T002: config-utils（並列実行可能） → T002-review（自動実行）
T003: firestore-deduplication（並列実行可能） → T003-review（自動実行）
  ↓
T004: rss-article-fetcher（並列実行可能） → T004-review（自動実行）
T005: gemini-integration（並列実行可能） → T005-review（自動実行）
T006: discord-notification（並列実行可能） → T006-review（自動実行）
  ↓
T007: main-orchestration → T007-review（自動実行）
  ↓
T008: local-testing → T008-review（自動実行）
  ↓
T009: gcp-deployment → T009-review（自動実行）

注: 各実装タスク完了後、自動的にレビュータスクが作成・実行されます
```
...
## T003: firestore-deduplication 🎯 [並列OK]
...
## T006: discord-notification 🎯 [並列OK]
...
## レビュータスク（Review Tasks）

以下のレビュータスクは、各実装タスクのPR作成後に独立したタスクとして作成・実行されます。

### T001R: infrastructure-review 🔍

**目的**: T001のTerraformインフラ構築をレビュー

**対象ブランチ**: `terraform/t001-infrastructure`

**レビュー内容**:
- [レビュー専用タスク](#レビュー専用タスク)のセクションを参照
- Terraformコードの品質、セキュリティ、ベストプラクティスを確認
- 特に注意: リソース命名規則、変数定義の妥当性、outputs定義の適切性

**完了条件**:
- [ ] PRの差分を全て確認
- [ ] レビュー観点に基づいた包括的なレビュー結果を作成
- [ ] ユーザーに結果を報告
- [ ] ユーザーの指示を待つ
...
### T002R: config-utils-review 🔍

**目的**: T002の設定管理とユーティリティをレビュー

**対象ブランチ**: `feat/t002-config-utils`

**レビュー内容**:
- [レビュー専用タスク](#レビュー専用タスク)のセクションを参照
- 設定スキーマの妥当性、エラーハンドリングの適切性を確認
- 特に注意: Secret Manager統合の安全性、ログ出力の適切性
...
### T003R: firestore-deduplication-review 🔍

**目的**: T003のFirestore重複排除機能をレビュー

**対象ブランチ**: `feat/t003-firestore-deduplication`

**レビュー内容**:
- [レビュー専用タスク](#レビュー専用タスク)のセクションを参照
- Firestoreスキーマとクエリの効率性を確認
- 特に注意: TTLロジックの正確性、エラーハンドリング、テストカバレッジ
...
### T004R: rss-article-fetcher-review 🔍

**目的**: T004のRSS記事取得機能をレビュー

**対象ブランチ**: `feat/t004-rss-article-fetcher`

**レビュー内容**:
- [レビュー専用タスク](#レビュー専用タスク)のセクションを参照
- RSSパース処理とgo-readability統合を確認
- 特に注意: エラーハンドリング、RSS 2.0とAtomの両対応、記事抽出の正確性
...
### T005R: gemini-integration-review 🔍

**目的**: T005のGemini評価機能をレビュー

**対象ブランチ**: `feat/t005-gemini-integration`

**レビュー内容**:
- [レビュー専用タスク](#レビュー専用タスク)のセクションを参照
- Gemini API統合とスコアリングロジックを確認
- 特に注意: レート制限の実装、プロンプトの妥当性、スコアリング重み付けの適切性
...
### T006R: discord-notification-review 🔍

**目的**: T006のDiscord通知機能をレビュー

**対象ブランチ**: `feat/t006-discord-notification`

**レビュー内容**:
- [レビュー専用タスク](#レビュー専用タスク)のセクションを参照
- Discord Webhook統合とEmbedsフォーマットを確認
- 特に注意: リトライロジック、ペイロードフォーマットの正確性、エラーハンドリング
...
### T007R: main-orchestration-review 🔍

**目的**: T007のメインオーケストレーションをレビュー

**対象ブランチ**: `feat/t007-main-orchestration`

**レビュー内容**:
- [レビュー専用タスク](#レビュー専用タスク)のセクションを参照
- エンドツーエンドフローの実装を確認
- 特に注意: エラーハンドリング、ログ出力、各コンポーネントの統合品質
...
### T008R: local-testing-review 🔍

**目的**: T008のローカルテストとデバッグをレビュー

**対象ブランチ**: `test/t008-local-testing`

**レビュー内容**:
- [レビュー専用タスク](#レビュー専用タスク)のセクションを参照
- テスト結果とバグ修正内容を確認
- 特に注意: テストカバレッジ、バグ修正の妥当性、ドキュメント更新
...
### T009R: gcp-deployment-review 🔍

**目的**: T009のGCPデプロイとCI/CDをレビュー

**対象ブランチ**: `deploy/t009-gcp-deployment`

**レビュー内容**:
- [レビュー専用タスク](#レビュー専用タスク)のセクションを参照
- デプロイ設定とCI/CDパイプラインを確認
- 特に注意: セキュリティ設定、Cloud Buildの設定、監視設定の妥当性

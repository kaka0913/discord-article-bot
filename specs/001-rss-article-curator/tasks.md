...
2. **Vibe Kanbanにレビュー用タスクを自動作成**し、そのタスクを即座に開始
3. レビューコメントは1〜2行程度、日本語で記載
4. **重要**: ユーザーの承認を得るまで絶対にマージしないこと

**レビュータスクの作成方法**:

PRを作成したら、以下の手順でVibe Kanbanにレビュー用タスクを作成し、即座に実行します：

1. **タスクタイトル**: `T{番号}-review: {元のタスク名}のレビュー`
   - 例: `T003-review: Firestore重複排除機能のレビュー`

2. **タスク説明**: 元のタスクの説明全体 + 以下のレビュー指示を追加
...
### レビュー対象
このタスクは、T{番号}（{タスク名}）のPRをレビューするためのタスクです。

### PR情報
- ブランチ: `{ブランチ名}`
- PR URL: {gh pr createで作成されたPR URL}
...
3. **タスク作成後、即座にそのタスクを開始する**
   - `mcp__vibe_kanban__start_task_attempt`を使用してタスクを開始
...
- [x] T001: Terraformインフラ構築 ✅ **完了**
- [x] T002: 設定管理とユーティリティ（並列OK） ✅ **完了**
- [ ] T003: Firestore重複排除機能（並列OK） 🔄 **次のタスク**
- [ ] T004: RSS記事取得機能（並列OK） 🔄 **次のタスク**
- [ ] T005: Gemini評価機能（並列OK） 🔄 **次のタスク**
- [ ] T006: Discord通知機能（並列OK） 🔄 **次のタスク**
- [ ] T007: メインオーケストレーション
- [ ] T008: ローカルテストとデバッグ
- [ ] T009: GCPデプロイとCI/CD
